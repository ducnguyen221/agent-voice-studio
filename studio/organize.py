...
organize.py — sắp xếp thư mục voices/ theo từng profile, và archive file cũ.
  ...
  python organize.py --keep narrator narrator voice-b voice-c speaker-b "speaker-c"
  python organize.py --keep ... --apply      # không có --apply = chỉ xem nhấp nháy, không đụng gì

## Vì sao KHÔNG chuyển .wav vào thư mục con

`voice_profiles.py` đọc đúng `voices/<tên>.wav` + `<tên>.txt`, và các pipeline cùng tác vụ định kỳ
...
def profile_names():
    ...
    return sorted(n[:-4] for n in os.listdir(VOICES)
                  if n.endswith(".wav") and not n.startswith("_")
                  and os.path.isfile(os.path.join(VOICES, n)))
...
def main():
    ...
    arch = os.path.join(VOICES, "_archive", stamp)
    dry = not args.apply
    ...
    with open(os.path.join(VOICES, "_layout.json"), "w", encoding="utf-8") as f:
        json.dump({"organized_at": stamp, "keep": sorted(keep),
                   "archived": plan_arch,
                   "note": "wav/txt phải ở PHẲNG — voice_profiles.py đọc voices/<tên>.wav"},
