...
build.py — dựng profile đa sắc thái từ clip ứng viên đã đào (DESIGN.md mục 8).
  ...
  python build.py --name narrator --mode new
  python build.py --name speaker-b-style2 --mode add        # nạp thêm marker, không phá cũ
...
def load_candidates(name):
    p = os.path.join(HERE, "work", name, "candidates.json")
    if not os.path.isfile(p):
        sys.exit(f"[build] chưa có {p} — chạy mine.py trước.")
...
def commit(stage, name, neutral_marker):
    ...
    vdir = os.path.join(VOICES, name + ".variants")
    ...
    for fn in sorted(os.listdir(stage)):
        ...
        if fn.endswith(".wav"):   # prompt cache của clip vừa thay đã lỗi thời
            ...
            if os.path.isfile(pt):
                os.remove(pt)
    if neutral_marker:
        # Clip trung tính lưu THẲNG ở voices/<name>.wav để voice_profiles.py CŨ đọc được.
        src = os.path.join(stage, neutral_marker + ".wav")
        if os.path.isfile(src):
            shutil.copyfile(src, os.path.join(VOICES, name + ".wav"))
            shutil.copyfile(os.path.join(stage, neutral_marker + ".txt"),
                            os.path.join(VOICES, name + ".txt"))
            pt = os.path.join(VOICES, name + ".prompt.pt")
            if os.path.isfile(pt):
                os.remove(pt)
...
def main():
    ...
    legacy_wav = os.path.join(VOICES, args.name + ".wav")
    ...
    added, skipped = [], []
    stage = os.path.join(HERE, "work", args.name, "_stage")
    ...
    if parts:
        ...
        ov._save(np.concatenate(seq), p, sr)
