...
def _voices():
    """Kho giọng hiện hành (đọc mỗi lần — không đóng băng lúc import)."""
    return profiles.voices_dir()


def _log(msg):
    # stderr: stdout của CLI dành cho kết quả máy đọc (hợp đồng --json).
...
def get_prompt(model, profile_name, profile, marker):
    ...
    if profile is None or marker is None:
        ...
        # thì ai quên xoá .pt sẽ được phục vụ ref_text CŨ, im lặng. module profile cũ đã
...
def synth(model, text, profile_name, language="Vietnamese", speed=None, verbose=True,
    ...
    if unknown and verbose:
        _log(f"[vlab] bỏ qua bracket lạ: {sorted(set(unknown))}")
    if not spans:
    ...
    for marker, chunk in spans:
        ...
        for sent in split_sentences(chunk):
            if seed is not None:
                ...
                # đúng một mẫu nhiễu.
                engine.seed_all(seed + len(parts))
            ...
            prev_marker = eff
            if verbose:
                _log(f"[vlab] {eff or '(gốc)':<12} {len(audio)/model.sampling_rate:5.2f}s  {sent[:52]!r}")
